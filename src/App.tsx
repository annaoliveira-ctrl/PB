...
export default function App() {
  // State for Form Inputs - only raw text input now
  ...
  const handleProcessText = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errData.error || "Erro ao processar a revisão de texto.");
      }

      const data: RevisedHandover = await response.json();
      setResult(data);
  ...
  return (
    <div id="pb-bot-expert-root" className="min-h-screen mesh-gradient text-slate-100 font-sans antialiased flex flex-col">
      ...
      <div id="main-container" className="flex-1 max-w-6xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col gap-6">
        ...
        {/* Banner with clear user guidance */}
          ...
          <section id="results-section" className="lg:col-span-7 flex flex-col glass-panel rounded-2xl p-5 shadow-2xl min-h-[450px] border border-white/10">
            ...
            {!result && !isLoading && (
              <div id="results-empty-state" className="flex-1 flex flex-col items-center justify-center text-center p-6 text-slate-400">
                ...
                {/* Visual Checklist showing user what the bot is designed to structure */}
            ...
            {/* Loading Progressive Step Panel */}
            {isLoading && (
            ...
            {result && !isLoading && (
              <div id="results-output" className="space-y-5 flex-1 overflow-y-auto pr-1 custom-scrollbar">
                ...
                {/* Gap Warning / Notas Pendentes (if any) */}
                ...
                <div id="mandatory-sections" className="space-y-4">
                  ...
                  {/* Section 4: Nivel Autonomia */}
                  <div className="border border-white/10 rounded-2xl bg-white/5 overflow-hidden hover:border-white/20 transition-all">
                    <div className="bg-white/5 px-4 py-2.5 border-b border-white/15 flex items-center justify-between">
                      <div className="flex items-center gap-2 font-bold text-xs text-indigo-300">
                        <span>4. Nível de Autonomia e Conhecimento</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <button
                ...
                {/* Bottom Copy Action Banner */}
                <div className="bg-white/5 rounded-2xl p-4 border border-white/10 flex flex-col sm:flex-row items-center justify-between gap-3 shadow-lg">
                  <div className="flex items-center gap-2 text-slate-400 text-xs text-left">
                    <Info className="w-4 h-4 text-indigo-400 flex-shrink-0" />
                    <span>Qualquer campo editado acima é atualizado dinamicamente para cópia ou download.</span>
                  </div>
                  <button
                    id="copy-report-bottom-btn"
                    onClick={handleCopyAll}
                    className="w-full sm:w-auto px-6 py-2.5 text-xs font-bold text-white bg-emerald-600 hover:bg-emerald-500 rounded-lg flex items-center justify-center gap-2 shadow-lg shadow-emerald-600/10 hover:shadow-emerald-600/25 cursor-pointer transition-all active:scale-98"
                  >
                    {copiedAll ? <Check className="w-4 h-4 text-white" /> : <Copy className="w-4 h-4 text-white" />}
                    {copiedAll ? "Copiado com sucesso!" : "Copiar Relatório Completo"}
                  </button>
                </div>
      ...
      {/* Footer */}
      <footer id="app-footer" className="bg-black/20 border-t border-white/5 py-6 text-center text-xs text-slate-500 mt-auto">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="flex gap-4">
            <span>© 2026 PB Bot Expert</span>
            <span>Fluxo Padronizado Ativação ➔ Adoção</span>
          </div>
          <div className="flex items-center gap-2">
            <span>Processado via Gemini 3.5 Flash Engine</span>
...
// Helper to map basic Markdown bold and list characters into safe HTML presentation
function formatMarkdownToHtml(markdown: string): string {
  if (!markdown) return "";
  
  let formatted = markdown;
  
  // Replace leading list stars/dashes (e.g. "* item" or "- item")
  formatted = formatted.replace(/^[-\*]\s+(.*)$/gm, '<div class="flex items-start gap-2 my-0.5"><span class="text-indigo-400 font-bold">•</span><span>$1</span></div>');
  
  // Replace bold identifiers **text** with elegant tags
